...
function renderChiamata(chiamata) {
  ...
  if (chiamata.aspettandoConferma) {
    ...
    if (S.asta.popupAttivo.tipo === "svincolo") {
      attesaBadge = "<p class=\"cc-attesa-badge\" style=\"color:var(--danger);border-color:var(--danger);background:rgba(239,68,68,0.1)\">🚨 Asta in pausa: <strong>" + _escHtml(S.asta.popupAttivo.squadraVincitrice) + "</strong> sta svincolando</p>";
  ...
  const cc_header_html = (!S.isAdmin && window.innerWidth > 1200)
    ? '<div class="cc-header">' +
        '<div class="cc-avatar-utente-col"><div class="cc-avatar">' + _avatarSvg + '</div>' + '</div>' +
        '<div class="cc-info">' +
